...
# Fast metrics
@register_metric
def ppl2_wend_20K(opts):
    ppl = perceptual_path_length.compute_ppl(opts, num_samples=20000, epsilon=1e-4, space='w', sampling='end', crop=False, batch_size=2)
    return dict(ppl2_wend=ppl)

# General metrics
@register_metric
def lpips_dist(opts):
    lpips_dist = lpips_distance.compute_lpips_distance(opts, num_samples=20000, crop=False, batch_size=1)
    return dict(lpips_dist=lpips_dist)
