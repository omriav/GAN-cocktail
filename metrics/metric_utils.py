...
class MetricOptions:
    def __init__(self, G=None, G_kwargs={}, dataset_kwargs={}, num_gpus=1, rank=0, device=None, progress=None, 
                cache=True, evaluating_class=None, num_classes=None, use_data_labels=False):
        ...
        self.cache          = cache
        self.evaluating_class = evaluating_class
        self.num_classes = num_classes
        self.use_data_labels = use_data_labels
...
def compute_feature_stats_for_generator(opts, detector_url, detector_kwargs, rel_lo=0, rel_hi=1, batch_size=64, batch_gen=None, jit=False, **stats_kwargs):
    ...
    while not stats.is_full():
        ...
        for _i in range(batch_size // batch_gen):
            ...
            if opts.use_data_labels:
                rand_classes = torch.randint(opts.num_classes, (batch_gen,))
                c = F.one_hot(rand_classes, num_classes=opts.num_classes).pin_memory().to(opts.device)
            else:
                c = F.one_hot(torch.tensor(opts.evaluating_class), num_classes=opts.num_classes).pin_memory().to(opts.device)
                c = c.repeat(batch_gen, 1)
