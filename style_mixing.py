...
def generate_style_mix(
    ...
    label_row = torch.zeros([1, G.c_dim], device=device)
    label_row[0][0] = 1
    label_col = torch.zeros([1, G.c_dim], device=device)
    label_col[0][1] = 1
    all_z = np.stack([np.random.RandomState(seed).randn(G.z_dim) for seed in all_seeds])
    all_label = torch.cat([label_row.repeat(len(row_seeds), 1), label_col.repeat(len(col_seeds), 1)])
    all_w = G.mapping(torch.from_numpy(all_z).to(device), all_label)
    ...
    for i, img in enumerate(all_images):
        PIL.Image.fromarray(img, 'RGB').save(f'{outdir}/source_{i}.png')
    ...
    for r, row_seed in enumerate(row_seeds):
        for c, col_seed in enumerate(col_seeds):
            ...
            PIL.Image.fromarray(image, 'RGB').save(f'{outdir}/r_{r}-c_{c}.png')
