...
def subprocess_fn(rank, args, temp_dir):
    ...
    for metric in args.metrics:
        if rank == 0 and args.verbose:
            print(f'Calculating {metric} for class {args.evaluating_class}...')
        progress = metric_utils.ProgressMonitor(verbose=args.verbose)
        result_dict = metric_main.calc_metric(metric=metric, G=G, dataset_kwargs=args.dataset_kwargs,
            num_gpus=args.num_gpus, rank=rank, device=device, progress=progress, 
            evaluating_class=args.evaluating_class, num_classes=args.num_classes)
        if rank == 0:
...
@click.option('--evaluating_class', help='The class number to evaluate the metric on', type=int, default=0)
@click.option('--num_classes', help='The class number to evaluate the metric on', type=int, default=2)
@click.option('--verbose', help='Print optional information', type=bool, default=True, metavar='BOOL', show_default=True)

def calc_metrics(ctx, network_pkl, metrics, data, mirror, gpus, evaluating_class, num_classes, verbose):
    """Calculate quality metrics for previous training run or pretrained network pickle.
    ...
    args.evaluating_class = evaluating_class
